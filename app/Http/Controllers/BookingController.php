...
class BookingController extends Controller
{
    ...
    public function printexportpdf(Request $request)
    {
        ...
        return view('pages.backend.booking.components.printexportpdf', compact('room_online_income_tax', 'total_cashpayment', 'total_onlinepayment', 'income_total', 'expence_total', 'income', 'expence', 'branch', 'managername', 'from_date', 'to_date', 'checkin_Array', 'checkout_Array', 'room_cash_income', 'room_online_income', 'room_cash_income_tax', 'total_gst'));
    }
    ...
    public function billtobill_printexportpdf(Request $request)
    {
        ...
        $checkin_Array = [];
        $room_list = [];
        foreach (($bookingPaymentArray) as $key => $bookingPaymentArrays) {

            $checkin_Data = Booking::where('check_in_date', '=', $bookingPaymentArrays->paid_date)->where('id', '=', $bookingPaymentArrays->booking_id)->where('soft_delete', '!=', 1)->first();
            if($checkin_Data != ""){
            ...
            if($bookingPaymentArrays->payment_method == 'Cash'){
                $cash_income = $bookingPaymentArrays->payable_amount;
            }
            if($bookingPaymentArrays->payment_method == 'Online Payment'){
                $online_income = $bookingPaymentArrays->payable_amount;
            }
            ...
            $checkin_Array[] = array(
                ...
                'paidDate_arrays' => $bookingPaymentArrays->paid_date,
                'room_list' => $room_list,
                'id' => $booking_id,
                'cash_income' => $cash_income,
